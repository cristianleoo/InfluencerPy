...
class Post:
    ...
    def __init__(self, url: str, auth: Optional[SubstackAuth] = None, cookies_dict: Optional[Dict] = None) -> None:
        """
        ...
        cookies_dict : Optional[Dict]
            Dictionary with cookie values: {'sid': '...', 'lli': '...'} (alternative to auth)
        """
        self.url = url
        
        # Create auth from cookies_dict if provided and no auth exists
        if cookies_dict and not auth:
            from .auth import SubstackAuth
